    ...
    <div className="min-h-screen bg-[#FBF9F6] flex flex-col pb-20 md:pb-0 md:pl-64">
      ...
      <main className="flex-1 p-4 md:p-8 max-w-[1184px] mx-auto w-full space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_300px] gap-6">
          ...
          <aside className="space-y-5 lg:sticky lg:top-20 lg:self-start">
            {/* Summary card */}
            <div className="bg-white border border-[#EAE3D5] rounded-2xl shadow-xs p-5 space-y-3">
            ...
            {/* Top categories card */}
            <div className="flex flex-col w-full h-[260px] bg-white border border-[#EFEBE7] rounded-[18px] p-[22px] shadow-[0_8px_30px_rgba(58,38,16,0.05),0_2px_6px_rgba(0,0,0,0.03)]">
              <h3 className="text-xl font-bold text-[#2E2E2E] mb-3">Categorias mais relatadas</h3>
                  ...
                  {topCategories.map(([cat, count]) => {
                    ...
                    return (
                      <div
                        ...
                        className={`group flex items-center justify-between h-9 ${isAdmin ? 'cursor-pointer rounded-xl px-2 -mx-2 hover:bg-[#F7F4F0] transition-colors' : ''}`}
                      >
                        <div className="flex items-center gap-3.5 min-w-0">
                          ...
                          <span className="text-[15px] font-medium text-[#2D2D2D] truncate">{cat}</span>
                        </div>
                        <span className="text-[17px] font-medium text-[#7A7A7A] shrink-0">{count}</span>
                      </div>
                    );
                  })}
                </div>
      ...
      {editingOcc && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div onClick={() => setEditingOcc(null)} className="absolute inset-0 bg-[#3E342F]/40 backdrop-blur-xs" />
          <motion.div
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div onClick={() => setDeleteConfirmId(null)} className="absolute inset-0 bg-[#3E342F]/40 backdrop-blur-xs" />
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.95 }}
      ...
      {summaryModal && isAdmin && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div onClick={() => setSummaryModal(null)} className="absolute inset-0 bg-[#3E342F]/40 backdrop-blur-xs" />
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.95 }}
        ...
        {toastOcc && (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-bold text-[#CBBFB7] uppercase tracking-wider">Notificação · Push</p>
